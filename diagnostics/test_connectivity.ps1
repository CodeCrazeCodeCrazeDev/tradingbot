...
try {
    ...
    if ($ping) {
        Write-Host '  ✅ Ping: Working (Google DNS reachable)' -ForegroundColor Green
        $results.ping = $true
    } else {
    ...
    Write-Host '  ⚠️ Ping: Error' -ForegroundColor Yellow
    Write-Host "     Error: $($_.Exception.Message)" -ForegroundColor Gray
    $results.issues += "Ping error: $($_.Exception.Message)"
}
...
try {
    ...
    Write-Host "  ✅ HTTPS: Working (Status: $($web.StatusCode))" -ForegroundColor Green
    $results.https = $true
} catch {
    Write-Host '  ❌ HTTPS: Failed' -ForegroundColor Red
    Write-Host "     Error: $($_.Exception.Message)" -ForegroundColor Gray
...
try {
    ...
    Write-Host "  ✅ Alpha Vantage: Reachable (Status: $($av.StatusCode))" -ForegroundColor Green
    $results.api = $true
} catch {
    Write-Host '  ⚠️ Alpha Vantage: Issue' -ForegroundColor Yellow
    Write-Host "     Error: $($_.Exception.Message)" -ForegroundColor Gray
...
try {
    ...
    Write-Host "  ✅ Yahoo Finance: Reachable (Status: $($yf.StatusCode))" -ForegroundColor Green
} catch {
    Write-Host '  ⚠️ Yahoo Finance: Issue' -ForegroundColor Yellow
    Write-Host "     Error: $($_.Exception.Message)" -ForegroundColor Gray
}
...
try {
    ...
    if ($proxyUri.ToString() -eq 'https://www.google.com/') {
        Write-Host '  ✅ No proxy configured (direct connection)' -ForegroundColor Green
    } else {
        Write-Host "  ⚠️ Proxy detected: $proxyUri" -ForegroundColor Yellow
        $results.issues += "Proxy configured: $proxyUri"
    }
} catch {
...
if ($results.issues.Count -eq 0) {
    ...
    Write-Host ''
    Write-Host '⚠️ CONNECTIVITY: PARTIAL' -ForegroundColor Yellow
...
if ($results.https -and $results.dns) {
    Write-Host "   Core connectivity is working" -ForegroundColor Green
    Write-Host "   Bot can operate normally in paper mode" -ForegroundColor Green
    if (-not $results.api) {
        Write-Host "   External API access may be limited" -ForegroundColor Yellow
