...
class CognitiveSystemController:
    ...
    def __init__(
        ...
        if getattr(self, "_initialized", False):
            # Dynamic updates of fields for backward compatibility testing
    ...
    async def process_market_observation(self, observation: Any) -> Optional[CoreDecision]:
        ...
        for branch in branches:
            ...
            if sim_func is None:
                # Fallback to simulate for legacy mock world model
                sim_func = getattr(self.world_model, "simulate", None)

            if sim_func:
                if asyncio.iscoroutinefunction(sim_func):
                    sim_results[branch.branch_id] = await sim_func(
                        obs_dict, branch.execution_plan, latent_z=latent_z
                    )
                else:
                    sim_results[branch.branch_id] = sim_func(
                        obs_dict, branch.execution_plan, latent_z=latent_z
                    )
            else:
        ...
        if self.shield is not None:
            shield_report = await self.shield.validate_action("trade", decision_proposal, {"market": obs_dict})
