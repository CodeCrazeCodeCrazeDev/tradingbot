...
class CognitiveSystemController:
    ...
    def __init__(self, *args, **kwargs):
        # Unify legacy (world_model, hms, shield) vs. standard (world_model, hms, skill_router, verifier_swarm, risk_engine, consensus_engine, execution_planner, evolution_gate, shield=None)
        ...
        if len(args) == 3:
            ...
            arg_names = [
                "world_model", "hms", "skill_router", "verifier_swarm",
                "risk_engine", "consensus_engine", "execution_planner",
                "evolution_gate", "shield"
            ]
            for i, val in enumerate(args):
                if i < len(arg_names):
                    setattr(self, arg_names[i], val)
    ...
    def _detect_failure_severity(self, reports: List[VerifierReport]) -> str:
        ...
        if not invalid_reports:
            return "none"
        critical_count = sum(1 for r in invalid_reports if getattr(r, 'confidence', 0) >= 0.9)
        if critical_count >= 1 or len(invalid_reports) >= 2:
            return "critical"
        return "minor"
    ...
    async def process_market_observation(self, observation: Any) -> Optional[CoreDecision]:
        ...
        if isinstance(intervention, dict) and intervention.get("status") == "pf_intervention":
            pf_res = intervention.get("pf_result", intervention.get("result", {}))
            logger.warning(f"CSC-V6 Step 3: HASP PF Intervention: {intervention.get('reason', 'High volatility override')}")
            if pf_res.get("action") == "override_to_hold":
                return CoreDecision(
                    outcome=DecisionOutcome.TRADE_REJECTED,
                    trade_id=observation.get("trade_id", str(uuid4())) if hasattr(observation, "get") else str(uuid4()),
                    dominant_rejection_reason=f"HASP PF Intervention: {intervention.get('reason', 'High volatility override')}"
                )
        ...
        for branch in branches:
            if branch.branch_id not in sim_results:
                if hasattr(self.world_model, "simulate_intervention"):
                    try:
                        sim_res_call = self.world_model.simulate_intervention(
                            observation, branch.execution_plan, latent_z=latent_z
                        )
                        sim_results[branch.branch_id] = await self._safe_await(sim_res_call) or {}
                    except Exception as e:
        ...
        await self._safe_await(decision_bus.propose_action(log_action))
        ...
        if self.shield is not None:
            ...
            decision = getattr(shield_report, "decision", None)
        ...
        await self._safe_await(decision_bus.propose_action(action))
    ...
    def _select_optimal_action(self, branch: ReasoningBranch, simulations: Dict[str, Any]) -> Dict[str, Any]:
        ...
        base_qty = branch.execution_plan.get("quantity", 0.1) if hasattr(branch.execution_plan, "get") else 0.1
        slippage_penalty = 1.0 - (expected_slippage * 100)

        return {
            "trade_id": str(uuid4()),
            "symbol": branch.execution_plan.get("symbol", "BTC/USDT") if hasattr(branch.execution_plan, "get") else "BTC/USDT",
            "action": branch.execution_plan.get("action", "WAIT") if hasattr(branch.execution_plan, "get") else "WAIT",
    ...
    def _create_ledger_entry(self, branch: ReasoningBranch, scenarios: Any) -> ResearchLedgerEntry:
