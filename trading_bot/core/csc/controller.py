...
class CognitiveSystemController:
    ...
    def __init__(self, *args, **kwargs):
        ...
        if verifier_swarm is None:
            ...
            class MockVerifierSwarm:
                async def verify_all(self, *args, **kwargs):
                    return type("MockOutcome", (), {"is_valid": True, "rejections": []})()
        ...
        decision_bus = kwargs.get("decision_bus")
    ...
    async def process_market_observation(self, observation: Any) -> Optional[CoreDecision]:
        ...
        if intervention.get("status") == "pf_intervention":
            pf_res = (
                intervention.get("pf_result", {})
                if isinstance(intervention, dict)
                else getattr(intervention, "pf_result", {})
            )
            reason = (
                pf_res.get("reason")
                if isinstance(pf_res, dict)
                else getattr(pf_res, "reason", "Unknown PF Intervention")
            )
            if not reason:
                reason = (
                    intervention.get("reason", "Unknown PF Intervention")
                    if isinstance(intervention, dict)
                    else getattr(intervention, "reason", "Unknown PF Intervention")
                )
            action = (
                pf_res.get("action") if isinstance(pf_res, dict) else getattr(pf_res, "action", "")
            )
            if not action:
                action = (
                    intervention.get("action", "")
                    if isinstance(intervention, dict)
                    else getattr(intervention, "action", "")
                )
        ...
        ledger_entry = self._create_ledger_entry(
            best_branch, sim_results.get(best_branch.branch_id, [])
        )
        ...
        shield_report = await self.shield.validate_action(
            "trade", decision_proposal, {"market": observation}
        )
    ...
    def _create_ledger_entry(
        self, branch: ReasoningBranch, scenarios: List[Any]
    ) -> ResearchLedgerEntry:
        provenance = {"timestamp": datetime.utcnow().isoformat(), "git_commit": "unknown"}
    ...
    async def _refine_strategy(self, branch: ReasoningBranch, reports: List[Any]) -> ReasoningBranch:
        """
        AutoResearchClaw Refinement Loop (arXiv:2605.20025).
        Inputs: branch (ReasoningBranch), verifier reports (List[VerifierReport]).
        Outputs: refined branch with adjusted confidence and recorded corrections.
        """
        logger.info(f"CSC-V6: Refining strategy branch {branch.branch_id}")

        # Deepcopy or recreate to ensure deterministic outputs
        import copy
        refined = copy.deepcopy(branch)

        # Refinement reduces confidence via a factor of 0.9
        refined.confidence = round(branch.confidence * 0.9, 3)

        # Record structured corrections and telemetry
        for report in reports:
            critique = getattr(report, "critique", "Unknown risk")
            refined.reasoning_trace.append(f"Correction: {critique}")

        logger.info(f"CSC-V6 Refined: New confidence={refined.confidence:.3f}, Trace={refined.reasoning_trace}")
        return refined
