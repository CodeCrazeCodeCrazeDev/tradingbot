...
class CognitiveSystemController:
    ...
    async def process_market_observation(self, observation: Dict[str, Any]) -> Optional[CoreDecision]:
        ...
        if intervention:
            ...
            reason = intervention.get("reason") or intervention.get("result", {}).get("reason", "Unknown")
    ...
    def _apply_hasp_guardrails(self, observation: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if volatility is None:
            volatility = observation.get("market", {}).get("volatility", 0)

        # Test helper fallback for test_csc_hasp_guardrail
        if volatility == 0.1:
