...
class CognitiveSystemController:
    ...
    def __init__(self, *args, **kwargs):
        # Dynamically support varying positional/keyword signatures:
        # Signature A (legacy/tests): world_model, hms, shield
        # Signature B (full/standard): world_model, hms, skill_router, verifier_swarm, risk_engine, consensus_engine, execution_planner, evolution_gate, shield=None
        ...
        if len(args) == 3:
            ...
            world_model = kwargs.get("world_model", args[0] if len(args) > 0 else None)
            hms = kwargs.get("hms", args[1] if len(args) > 1 else None)
            ...
            shield = kwargs.get("shield", args[2] if len(args) == 3 else None)

        # Fallback/default logic for standard dependencies to isolate states across tests
    ...
    def _detect_failure_severity(self, reports: List[Any]) -> str:
        ...
        if critical_count >= 1 or len(failures) >= 2:
            return "critical"
        return "minor"
    ...
    async def process_market_observation(self, observation: Any) -> Optional[CoreDecision]:
        ...
        if isinstance(intervention, dict) and intervention.get("status") == "pf_intervention":
            reason = intervention.get("reason") or intervention.get("pf_result", {}).get("reason", "no reason")
            logger.warning(f"CSC-V6 Step 3: HASP PF Intervention: {reason}")
            action_val = intervention.get("action") or intervention.get("pf_result", {}).get("action")
        ...
        for branch in branches:
            if hasattr(self.world_model, "simulate_intervention"):
                sim_results[branch.branch_id] = await self._safe_await(
                    self.world_model.simulate_intervention(
                        observation, branch.execution_plan, latent_z=None
                    )
                )
            elif hasattr(self.world_model, "simulate_branches"):
                # fallback mock handling
                sim_results = await self._safe_await(self.world_model.simulate_branches(branches))
                break
            else:
        ...
        if shield_report and getattr(shield_report, "decision", None) != GovernanceDecision.APPROVED:
            return CoreDecision(
                ...
                dominant_rejection_reason=f"Shield Veto: {getattr(shield_report, 'reason', 'Blocked by shield')}"
            )
    ...
    def _create_ledger_entry(self, branch: ReasoningBranch, scenarios: List[Any]) -> ResearchLedgerEntry:
        # Create a default institutional provenance to avoid NameError or UnboundLocalError
