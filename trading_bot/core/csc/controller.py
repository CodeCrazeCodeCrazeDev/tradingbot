...
class CognitiveSystemController:
    ...
    def __init__(self, *args, **kwargs):
        # Always re-initialize inside tests to bind mocks correctly
        if getattr(self, "_initialized", False):
            # Update injected dependencies if passed in args/kwargs
            if len(args) == 3:
            ...
            for k, v in kwargs.items():
                if k in ["world_model", "hms", "skill_router", "verifier_swarm", "risk_engine", "consensus_engine", "execution_planner", "evolution_gate", "shield"]:
                    setattr(self, k, v)
        ...
        # Handling both legacy 3-positional (world_model, hms, shield)
        # and updated 8-positional (world_model, hms, skill_router, verifier_swarm, risk_engine, consensus_engine, execution_planner, evolution_gate, shield=None)
        if len(args) == 3:
            self.world_model = args[0]
            self.hms = args[1]
            self.shield = args[2]
        ...
        # Parse keyword arguments (overwriting if specified)
        for k, v in kwargs.items():
            if k in ["world_model", "hms", "skill_router", "verifier_swarm", "risk_engine", "consensus_engine", "execution_planner", "evolution_gate", "shield"]:
                setattr(self, k, v)

        # Apply fallback mocks if needed
    ...
    async def process_market_observation(self, observation: Any) -> Optional[CoreDecision]:
        ...
        # Normalize the input observation using MarketContextAdapter if it's not a dict, or ensure dictionary
        ...
        logger.info(f"CSC-V6 Step 4: DiscoLoop complete.")

        # 5. Multi-Hypothesis Generation (AutoResearchClaw)
        branches = await self.hypothesis_gen.generate_competing_branches(obs_dict)
        ...
        for branch in branches:
            try:
                sim_res = await self._safe_await(self.world_model.simulate_intervention(
                    obs_dict, branch.execution_plan, latent_z=latent_z
                ))
            except Exception:
                sim_res = {"failure_rate": 0.1, "expected_slippage": 0.001, "structural_impact": {}}
            sim_results[branch.branch_id] = sim_res
            ...
            try:
                ...
                if shield_report and getattr(shield_report, "decision", GovernanceDecision.APPROVED) != GovernanceDecision.APPROVED:
                    return CoreDecision(
                        ...
                        dominant_rejection_reason=f"Shield Veto: {getattr(shield_report, 'reason', 'unknown reason')}"
                    )
            except Exception as e:
