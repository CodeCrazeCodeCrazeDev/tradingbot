...
class CognitiveSystemController:
    ...
    @classmethod
    def reset(cls):
        """Reset the singleton instance state for test isolation."""
        with cls._lock:
            cls._instance = None
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Support both legacy 3-positional signatures and standard 8/9-positional bindings
        # Position 0: world_model
        # Position 1: hms
        # Position 2: shield (or skill_router/shield depending on signature)

        # Parse positional args
        world_model = args[0] if len(args) > 0 else kwargs.get("world_model")
        hms = args[1] if len(args) > 1 else kwargs.get("hms")

        # Depending on count, the third argument might be shield (legacy) or skill_router (standard)
        skill_router = None
        shield = None
        ...
        if len(args) == 3:
            # Legacy 3 positional arguments: world_model, hms, shield
            shield = args[2]
        elif len(args) >= 8:
        ...
        # 1. Dependency Injection / Self Fallbacks
    ...
    async def process_market_observation(self, observation: Any) -> Optional[CoreDecision]:
        ...
        for branch in branches:
            try:
                ...
                # Mock or simulation-less fallback
                sim_results[branch.branch_id] = {"expected_slippage": 0.0, "structural_impact": {}, "failure_rate": 0.0}
    ...
    def _create_ledger_entry(self, branch: ReasoningBranch, scenarios: List[Any]) -> ResearchLedgerEntry:
        prov = InstitutionalProvenance(
            git_sha="csc_v6_sha",
            pipeline_version="UCA-V6",
            risk_policy_version="v2.0"
        )
