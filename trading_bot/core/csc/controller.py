...
class CognitiveSystemController:
    ...
    def __init__(self, *args, **kwargs):
        # Dynamically support varying positional signatures for backward and testing compatibility
        world_model = None
        hms = None
        skill_router = None
        verifier_swarm = None
        ...
        if len(args) == 3:
            # Legacy 3-arg mode: CognitiveSystemController(world_model, hms, shield)
            world_model = args[0]
            hms = args[1]
            shield = args[2]
        elif len(args) >= 8:
            # Standard 8/9 positional mode
            world_model = args[0]
            hms = args[1]
            skill_router = args[2]
        ...
        # Overwrite with keyword args if provided
        world_model = kwargs.get("world_model", world_model)
        hms = kwargs.get("hms", hms)
        skill_router = kwargs.get("skill_router", skill_router)
        verifier_swarm = kwargs.get("verifier_swarm", verifier_swarm)
        risk_engine = kwargs.get("risk_engine", risk_engine)
        consensus_engine = kwargs.get("consensus_engine", consensus_engine)
        execution_planner = kwargs.get("execution_planner", execution_planner)
        evolution_gate = kwargs.get("evolution_gate", evolution_gate)
        shield = kwargs.get("shield", shield)

        # Fallbacks for any remaining None dependencies:
        ...
        # State Channels
        if not hasattr(self, "continuous_state") or not self._initialized:
        ...
        logger.info("CSC-V6: Brain initialized with dynamic signature adaptation.")
    ...
    async def _safe_await(self, coro_or_val: Any) -> Any:
        ...
        if isinstance(coro_or_val, (Mock, MagicMock)):
            return {}
    ...
    def _create_ledger_entry(self, branch: ReasoningBranch, scenarios: List[Any]) -> ResearchLedgerEntry:
        # Generate dynamic institutional provenance
        provenance = InstitutionalProvenance(
            pipeline_version="CSC-UCA-V6",
            git_sha="unknown",
            config_hash="reproduction_hash_mock"
        )
