...
class CognitiveSystemController:
    ...
    async def process_market_observation(self, observation: Dict[str, Any]) -> Optional[CoreDecision]:
        ...
        if intervention:
            ...
            if intervention.get("action") == "override_to_hold" or intervention.get("status") == "pf_intervention":
                return CoreDecision(
        ...
        if action.status != ActionStatus.EXECUTED:
            ...
            return CoreDecision(outcome=DecisionOutcome.TRADE_REJECTED, trade_id=trade_proposal.get("trade_id", "N/A"), dominant_rejection_reason=reason)
    ...
    def _apply_hasp_guardrails(self, observation: Dict[str, Any]) -> Dict[str, Any]:
        ...
        volatility = observation.get("volatility") or observation.get("market", {}).get("volatility", 0)
        if volatility > 0.3:
    ...
    async def _run_discoloop_internalization(self, observation: Dict[str, Any], num_loops: int = 2):
        """DiscoLoop dual-channel internalization logic."""
        latent = observation.get("latent_embedding", {})
    ...
    def _detect_failure_severity(self, reports: List[Any]) -> str:
        """Determines failure severity of verification swarm reports."""
        if not reports:
            return "none"
        invalid_reports = [r for r in reports if not getattr(r, "is_valid", True)]
        if not invalid_reports:
            return "none"
        high_confidence_failures = [r for r in invalid_reports if getattr(r, "confidence", 0.0) > 0.9]
        if len(high_confidence_failures) >= 1 or len(invalid_reports) >= 2:
            return "critical"
        return "minor"
