...
class CognitiveSystemController:
    ...
    async def process_market_observation(self, observation: Dict[str, Any]) -> Optional[CoreDecision]:
        ...
        if not best_branch:
            return CoreDecision(outcome=DecisionOutcome.TRADE_REJECTED, trade_id="no_branches", dominant_rejection_reason="No viable reasoning branches")
        ...
        if not decision_ready:
            return CoreDecision(outcome=DecisionOutcome.TRADE_REJECTED, trade_id="pivot_failed", dominant_rejection_reason="Failed Pivot/Refine loop")
        ...
        if status != ActionStatus.APPROVED and status != ActionStatus.EXECUTED:
            return CoreDecision(outcome=DecisionOutcome.TRADE_REJECTED, trade_id=trade_proposal.get("trade_id", "logact_failed"), dominant_rejection_reason=f"LogAct failure: {status}")
        ...
        if action.status != ActionStatus.EXECUTED:
            ...
            return CoreDecision(outcome=DecisionOutcome.TRADE_REJECTED, trade_id=trade_proposal.get("trade_id", "consensus_failed"), dominant_rejection_reason=reason)
    ...
    def _apply_hasp_guardrails(self, observation: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if vol is None and "market" in observation:
            if isinstance(observation["market"], dict):
        ...
        if vol > 0.3:
            ...
            if skill and skill.executable:
                # To ensure context contains market sub-dict if not present
