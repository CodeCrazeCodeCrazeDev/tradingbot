...
class CognitiveSystemController:
    ...
    async def process_market_observation(self, observation: Dict[str, Any]) -> Optional[CoreDecision]:
        ...
        if intervention:
            ...
            if intervention.get("status") == "pf_intervention" or "override_to_hold" in str(intervention):
                return CoreDecision(
        ...
        if not decision_ready:
            return CoreDecision(outcome=DecisionOutcome.TRADE_REJECTED, trade_id=str(best_branch.branch_id), dominant_rejection_reason="Failed Pivot/Refine loop")
    ...
    def _apply_hasp_guardrails(self, observation: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if volatility is None and "market" in observation:
