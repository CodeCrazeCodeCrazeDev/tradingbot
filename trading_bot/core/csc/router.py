...
class SkillRouteOutcome:
    ...
    def __getitem__(self, item):
        ...
        if hasattr(self, item):
            ...
            # Handle possible enum status or class types
            if item == "status" and hasattr(val, "value"):
                return val.value
            return val
        raise KeyError(item)
...
class HASPExecutor:
    ...
    async def execute(self, skill_id: str, state: Dict[str, Any], version: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if isinstance(res, dict) and "illegal_action" in res:
                logger.error(f"HASP: Post-execution invariant failure: illegal action '{res['illegal_action']}' detected!")
                return {"status": "invariant_fail", "reason": f"Illegal action '{res['illegal_action']}' vetoed by HASP executor."}
            return res
