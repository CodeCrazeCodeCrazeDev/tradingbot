...
class DualString(str):
    ...
    def __eq__(self, other):
        if str(self) in ("pf_intervention", "success"):
            return other in ("pf_intervention", "success")
        if str(self) in ("s2l_routed", "dispatched_to_adapter"):
...
class SkillRouter:
    ...
    def _initialize_default_skills(self):
        ...
        # Register standard S2L adapters - using lora_hedging_v1 for backward compatibility
        self.register_skill(SkillArtifact(
            ...
            if skill:
                if skill.skill_type == SkillType.LORA:
                    return {
                        ...
                        "result": {"adapter_id": skill.adapter_id}
                    }
