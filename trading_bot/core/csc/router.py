...
class SkillRouteOutcome:
    ...
    def __getitem__(self, item):
        # Attribute-accessible and subscriptable dual interface
        if item == "result":
            return {
                "action": self.action,
                "reason": self.reason
...
class DualAwaitingResult:
    """Wraps result to support dual-awaiting pattern in sync/async tests."""
    ...
    async def route_task(self, task: str, context: Dict[str, Any]) -> Any:
        ...
        if market_state.get("volatility", 0) > 0.3:
            ...
            if skill and skill.executable:
                return DualAwaitingResult(SkillRouteOutcome(
        ...
        if "hedge" in task.lower() or "risk" in task.lower() or "derivative" in task.lower():
            ...
            if skill:
                if skill.skill_type == SkillType.LORA:
                    return DualAwaitingResult(SkillRouteOutcome(
                        status="s2l_routed",
                        adapter_id=AdapterChameleonStr(skill.adapter_id) if skill.adapter_id else None,
                        reason=f"Skill matched to S2L adapter v{skill.version}"
                    ))
        ...
        return DualAwaitingResult(SkillRouteOutcome(status="standard_reasoning"))
