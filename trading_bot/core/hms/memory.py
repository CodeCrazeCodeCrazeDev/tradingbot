...
class HierarchicalMemorySystem:
    ...
    def _load_schema(self) -> Dict[str, Any]:
        ...
        if "migration_history" not in schema:
            schema["migration_history"] = []
        if "provenance" not in schema:
            schema["provenance"] = {"system": "AlphaAlgo_HMS", "creator": "CoreBrain"}
        if "integrity_hash" not in schema:
            schema["integrity_hash"] = self._calculate_integrity_hash(schema)

        return schema

    def _calculate_integrity_hash(self, schema: Dict[str, Any]) -> str:
        # Calculate deterministic SHA256 of entities, relations, version
        content = {
    ...
    def optimize_metamemory(self, success_trajectories: List[Any]):
        ...
        current_version = float(self.memory_schema.get("schema_version", "1.0"))
        target_version = f"{current_version + 0.1:.1f}"
