...
class AIBoundaryEnforcer:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.purpose_lock = PurposeLock(config.get('purpose', {}))
        self.meta_rules = MetaAlignmentRules(config.get('meta', {}))
        self.human_approval = HumanApprovalAbsolute(config.get('approval', {}))
        self.control = NeverOutgrowControl(config.get('control', {}))
