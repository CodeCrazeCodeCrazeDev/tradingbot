...
class AgentArgument:
    ...
    def __post_init__(self):
        # Prevent partially initialized state by enforcing high-quality defaults (Phase 2 standardization)
        if self.observation is None:
            self.observation = f"Observation check for {self.agent_role.value if hasattr(self.agent_role, 'value') else str(self.agent_role)}"
        if not self.evidence:
            self.evidence = [f"Metrics and state analyzed by {self.agent_role.value if hasattr(self.agent_role, 'value') else str(self.agent_role)}."]
        if self.hypothesis is None:
            self.hypothesis = f"Hypothesis regarding {self.action.value if hasattr(self.action, 'value') else str(self.action)} action."
        if not self.predictions:
            self.predictions = [f"Expect {self.action.value if hasattr(self.action, 'value') else str(self.action)} alignment with HTF structures."]
        if not self.counter_evidence:
            self.counter_evidence = ["Potential counter-trend volatility surge."]
        if self.verification is None:
            self.verification = "Verified via system-wide metric indicators."
    ...
    def analyze(self, context: MarketContext) -> AgentArgument:
        try:
            ...
            if total_score > 0.4:
                ...
                hypothesis = "Neutral macro HTF trend."
                predictions = ["Sideways range-bound oscillation between key levels."]
                counter_evidence = ["Structural breakout above key resistance or below key support."]
                verification = "HTF neutral range validated."
    ...
    def analyze(self, context: MarketContext) -> AgentArgument:
        try:
            ...
            if total_score > 0.3:
                ...
                hypothesis = "Neutral LTF trend without timing edge."
                predictions = ["Price expected to oscillate in local range."]
                counter_evidence = ["Breakout volume surge or sudden volatile range expansion."]
                verification = "Hold recommendation validated by lack of tactical momentum."
    ...
    def analyze(self, context: MarketContext) -> AgentArgument:
        try:
            ...
            if risk_flags >= 2:
                ...
                hypothesis = "Elevated multi-factor risk regime detected."
                predictions = ["Market is vulnerable to systemic downside shock."]
                counter_evidence = ["Systemic market stabilization or panic compression."]
                verification = "Veto confirmed by risk threshold parameters."
            elif risk_flags == 1:
                action = TradeAction.HOLD
                conviction = Conviction.HIGH
                ...
                hypothesis = "Moderate single-factor risk presence."
                predictions = ["Potential near-term consolidation or minor drawdown."]
                counter_evidence = ["Risk signal retracts back within normal bands."]
                verification = "Reduced-size trade or hold indicated."
            elif total_score > 0:
                action = TradeAction.HOLD  # Risk allows trading
                conviction = Conviction.MODERATE
                reasoning.append("✅ Risk parameters acceptable")
                hypothesis = "Nominal risk regime with acceptable capacity."
                predictions = ["Standard market execution has optimal safety buffer."]
                counter_evidence = ["Sudden intraday exposure exceedance."]
                verification = "All risk metrics fall safely within parameters."
            else:
                action = TradeAction.BUY
                conviction = Conviction.MODERATE
                anti_trade_reasoning.append("Sub-zero overall risk-adjusted fitness score")
                hypothesis = "Extremely low risk state - optimal fitness for alpha exposure."
                predictions = ["Bullish alpha execution has minimal friction."]
                counter_evidence = ["Unforeseen macro risk surge or system outage."]
                verification = "Double-checked risk and exposure limits remain green."
...
class HeadAI:
    ...
    def synthesize_decision(
        ...
        try:
            ...
            if len(active_arguments) == 3 and max(bullish, bearish, neutral) == 2:
...
class MultiAgentDebateSystem:
    ...
    async def debate(self, topic: Any, context: Optional[MarketContext] = None) -> FinalDecision:
        ...
        try:
            ...
            # If all core agents crashed/failed to respond, trigger emergency veto fail-closed (Phase 4 robust fail-closed)
            if all(arg.is_fallback for arg in current_round_args):
